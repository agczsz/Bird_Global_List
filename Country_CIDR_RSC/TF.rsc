/log info "Loading TF IPv4 Address List"
/ip firewall address-list
:do { add list=TF address=5.175.190.180/31 } on-error={}
:do { add list=TF address=44.32.192.82/32 } on-error={}
:do { add list=TF address=89.203.249.76/32 } on-error={}
:do { add list=TF address=89.203.250.51/32 } on-error={}
:do { add list=TF address=104.28.13.67/32 } on-error={}
:do { add list=TF address=104.28.13.68/31 } on-error={}
:do { add list=TF address=136.23.6.121/32 } on-error={}
:do { add list=TF address=136.23.20.49/32 } on-error={}
:do { add list=TF address=140.248.56.215/32 } on-error={}
:do { add list=TF address=140.248.57.215/32 } on-error={}
:do { add list=TF address=140.248.58.215/32 } on-error={}
:do { add list=TF address=140.248.59.215/32 } on-error={}
:do { add list=TF address=140.248.60.215/32 } on-error={}
:do { add list=TF address=140.248.61.215/32 } on-error={}
:do { add list=TF address=140.248.62.215/32 } on-error={}
:do { add list=TF address=140.248.63.215/32 } on-error={}
